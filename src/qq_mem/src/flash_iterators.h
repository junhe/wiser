...
class PositionPostingBagIterator {
 public:
  ...
  void SkipTo(int posting_bag) {
    ...
    GoToSkipInterval(skip_interval);
    int cur_bag = skip_interval * PACK_SIZE;

    int n_poses_to_hop = NumPositionsBetween(cur_bag, posting_bag);
    for 
  }

  int CozyEntryIndex() const {
    return cur_cozy_entry_index_;
  }
 ...
 private:
  int NumPositionsBetween(int bag_a, int bag_b) {
    int n = 0;
  ...
  void GoToSkipInterval(const int skip_interval) {
    SkipEntry &ent = skip_list_[skip_interval];
    off_t &blob_off = entry.file_offset_of_pos_blob;
    int in_blob_off = entry.in_blob_index_of_pos_bag;
    ...
    if (format == BlobFormat::PACKED_INTS) {
      pack_ints_iter_.Reset(blob_buf);
      pack_ints_iter_.SkipTo(in_blob_off);
    } else {
      vints_iter_.Reset(blob_buf);
      vints_iter_.SkipTo(in_blob_off);
    }

    cur_posting_bag_ = skip_list_.StartPostingIndex(skip_interval);
    cur_cozy_entry_index_ = skip_interval * PACK_SIZE + in_blob_off;
  }

  int FindSkipInterval(const int posting_bag) {
    int i = CurPostingInterval();
