...
def set_cgroup_mem_limit(n_bytes):
    print "setting cgroup memoryt to MB", n_bytes / MB
    shcmd("echo {} |sudo tee /sys/fs/cgroup/memory/charlie/memory.limit_in_bytes".format(n_bytes),
            ignore_error = True)
...
class Exp(Experiment):
    ...
    def before(self):
        ...
        set_es_yml(self.confs[0])
        set_jvm_options(self.confs[0])
        set_swap(os_swap)
        set_read_ahead_kb(read_ahead_kb)

        drop_cache()

        self.server_p = start_server(self.confs[0])
    ...
    def beforeEach(self, conf):
        ...
        set_cgroup_mem_limit(128*MB)
        time.sleep(2)
        set_cgroup_mem_limit(conf['server_mem_size'])
        time.sleep(2)

        # raw_input("Just dropped cache, you can check the size")
        ...
        print "Wait for client to fully start (3 sec)..."
        time.sleep(3)
    ...
    def treatment(self, conf):
        ...
        d["p_cache_min"] = min(cache_size_log)
        d['MB_read'] = mb_read
        d.update(conf)
        del d['query_path']
    ...
    def afterEach(self, conf):
        ...
        time.sleep(2)
        # raw_input("Record the MB_read and then continue...(y to continue, n to stop)")
