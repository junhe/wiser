...
// Posting_List Class
class Posting_List_Raw :  public PostingListStructure {
...
// Init with a term string, when creating index
    Posting_List_Raw(std::string term_in);    // do we need term?

// Init with a term string, and posting list string reference, when query processing
    Posting_List_Raw(std::string term_in, std::string & serialized_in);    // configuration
